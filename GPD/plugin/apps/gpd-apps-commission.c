...
// Build commissioning fram based on the GPD configuration.
// most of the feilds are compiltetime configuration.
                                            ...
                                            uint8_t* cmd)
{
...
#else
    memcpy(&(cmd[index]), gpd->securityKey, EMBER_AF_PLUGIN_APPS_KEY_LENGTH);
#endif
  }
  ...
  uint8_t *appInfo = &(cmd[index]); // remeber the application info location
...
#ifdef EMBER_AF_PLUGIN_APPS_APPS_GPD_MANUFACTURE_ID_PRESENT
...
#ifdef EMBER_AF_PLUGIN_APPS_APPS_GPD_MODEL_ID_PRESENT
...
// the security level incase of battery powered implementation.
                               ...
                               uint8_t secondNextChannel)
{
  ...
  // Channel togleing payload at index 1 :
...
// command out. It also takes the decission to set or clear the rxAfterTx if the current
...
//         (and no rxAfterTx incase of bidirectional)
// second : send the seriees of application description frames untill last but one frame is
//          sent out. Example, for 3 report descritors, lets say 3 application description frames.
...
// ----------------------------------Channel Request ------------------------------------------
// --------------------------------------------------------------------------------------------
// Thsi block of code is for Channel request by teh GPD to find the operational channel of the
// Proxy/Combo in case of bidirectional commissioninging.
#if (defined EMBER_AF_PLUGIN_APPS_BIDIR_CAPABLE) && (EMBER_AF_PLUGIN_APPS_BIDIR_CAPABLE == true)
...
// Sends the channle srequest on the rx channnel with autocommissioning cleared and rxAfterTx set
...
// first : Send the channel request with channel toggleing behaviour (includes rx channel)

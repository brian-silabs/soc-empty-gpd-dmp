...
void emberGpdLoadGpdDefaultConfiguration(EmberGpd_t * gpd)
{
  ...
  #else
  uint8_t *ieee = (uint8_t*)(&SYSTEM_GetUnique());
  #endif
  memcpy(gpd->addr.id.ieee, ieee, EMBER_GPD_EUI64_SIZE);
  gpd->addr.gpdEndpoint = EMBER_AF_PLUGIN_APPS_GPD_ENDPOINT;
#else
#error "Unsupported GPD Application Id"
  ...
  memcpy(gpd->securityKey, defaultKey, EMBER_AF_PLUGIN_APPS_KEY_LENGTH);

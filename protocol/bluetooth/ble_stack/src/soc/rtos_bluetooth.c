...
{
  RTOS_ERR      os_err;
  OS_FLAGS      flags = BLUETOOTH_EVENT_FLAG_EVT_HANDLED;
  ...
  while (DEF_TRUE) {
    ...
    if (gecko_event_pending() && (flags & BLUETOOTH_EVENT_FLAG_EVT_HANDLED)) {  //update bluetooth & read event
      bluetooth_evt = gecko_wait_event();
      APP_RTOS_ASSERT_DBG(bluetooth_evt, 1);
